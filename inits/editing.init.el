...
(defun my-deactivate-region-and-call-original (orig-fun &rest args)
  "Deactivate region if needed, then call ORIG-FUN with ARGS."
  ;; まずリージョン解除
  (when (and (not (memq last-input-event
                        '(S-left S-right S-down S-up
                          C-S-left C-S-right C-S-down C-S-up
                          M-S-left M-S-right M-S-down M-S-up)))
             mark-active)
    (cond
...
(when is-deactivate-region
  (dolist (cmd '(previous-line next-line left-char right-char))
    (advice-add cmd :around #'my-deactivate-region-and-call-original)))
