...
- 何をしたか: `focus-on-editable-buffers` に成功通知用の `foeb/buffer-killed-message` を追加し、変更済みバッファの確認で y を選び、実際に `kill-buffer` が成功した場合だけ通知するようにした。英語の既定値は `Buffer killed.`、個人設定側は「バッファを閉じました。」とした。
- なぜそうしたか: y で閉じた後に確認文がエコーエリアへ残る状態を解消し、n の「バッファを閉じませんでした。」と対になる結果通知を表示するため。
- 未完了の事項: なし。GUI Emacs 上で期待どおり動作することを確認済み。
- 次にやるとよいこと: 通常運用で表示時間や文言が気になった場合に、設定値だけを調整する。

- 何をしたか: 独立パッケージ `focus-on-editable-buffers` の利用者向け確認・通知・Anything不在エラーを `defcustom` にし、英語を既定値に戻した。トップ側の `inits/anything.init.el` では、その3項目に日本語を設定した。
- なぜそうしたか: 配布・再利用するEmacs Lispとしては英語を基本にしながら、個人環境では日本語の使用感を試せるよう、パッケージと利用者設定の責務を分けるため。
- 未完了の事項: Anythingの候補見出し `Buffers` / `Dired Buffers` はメッセージではないため、今回の設定項目化には含めていない。
- 次にやるとよいこと: 日本語の確認文とキャンセル通知を実運用で確認し、必要なら候補見出しも別途カスタマイズ可能にする。
...
- 次にやるとよいこと: しばらく運用して、`s-q`、`s-w`、`C-s` 中の `<muhenkan>` が安定して無害化されることを確認する。問題がなければ、このブロックは当面固定でよい。
...
- 何をしたか: `inits/web-authoring-set.el` の通常タグ付与を拡張し、選択範囲がない場合は現在の物理行を対象に、外側タグの置換、プレーンテキストのタグ付与、末尾 `<br>` / `<br />` の除去とタグ付与を行うようにした。インデント・末尾空白・本文内のカーソル相対位置は維持し、属性付き外側タグの属性は置換時に破棄する。片側タグや開始・終了タグ不一致など曖昧な行は変更せず `user-error` にした。
- なぜそうしたか: HTMLタグを付け直す際に、行を選択したり既存の外側タグを手作業で削除したりする操作を省くため。特殊処理を持つリンク・画像・リスト・表などの従来動作へ影響させないため、通常タグの分岐だけを対象にした。
- 未完了の事項: GUI上の実キー（`s-M-*`）による体感確認は未実施。既存コードが利用する旧 `cl` の `find` 依存は今回変更していない。
- 次にやるとよいこと: 実際のHTMLバッファで見出し変更、プレーン行、`<br>` 行、属性付き外側タグを `s-M-*` から操作し、カーソル位置を含めて使い勝手を確認する。

- 何をしたか: `inits/web-authoring-set.el` の `ul-li` / `ol-li` 変換前に共通の項目正規化を追加し、行頭のインデント、`・`、`-`、数字、数字＋ピリオドのリストマーカーと、行末の `<br>` / `<br />` を除去するようにした。裸の数字は後続空白がある場合だけマーカーとして扱い、`2026年度` のような本文は維持する。
- なぜそうしたか: 行頭記号や改行タグを含む原稿テキストを、前処理なしでそのまま `s-M-u` / `s-M-o` によりHTMLリストへ変換できるようにするため。
- 未完了の事項: GUI上の実原稿に対する操作感は未確認。
- 次にやるとよいこと: 実際の日本語原稿で `・`、ハイフン、番号付き、末尾 `<br>` の各形式を選択して変換結果を確認する。

- 何をしたか: 選択範囲なしで現在行を通常タグへ置換するとき、対象範囲の削除後に明示的に置換開始位置へ移動してからタグを挿入するよう修正した。現在行判定のインデントと末尾空白で全角スペースを認識し、`<b>...</b><br>` / `<u>...</u><br>` 形式の疑似見出しも外側タグと `br` をまとめて置換対象にした。
- なぜそうしたか: キャレットが行頭またはインデント内にある場合、保持した行頭インデントより前へタグが挿入され、空白がタグの末尾へ移動していたため。また、半角・全角スペースが混在する行では外側タグを曖昧と判定し、古いHTMLで使われる `b` / `u` と `br` の疑似見出しも直接変換できなかったため。
- 未完了の事項: なし。各キャレット位置、全角スペース混在、属性・内側タグを含む `b` / `u` 疑似見出しをテストした。
- 次にやるとよいこと: 実バッファで `        （議員Ｂ）<br>`、`       　<b>関西広域連合議会議員</b>`、`<u>文字列</u><br>` を行頭から見出しへ変換し、期待どおりになることを確認する。

- 何をしたか: `inits/` の軽い監査結果から、FTP資格情報処理を除く指摘を修正した。バックアップ・自動保存・ロックファイルを有効化して権限 `0700` の `.tmp/` 配下へ集約し、junkファイルも `.tmp/junk/` へ移した。管理対象4ディレクトリ直下の30日超の通常ファイルだけを起動後idle時に削除するガベージコレクションを追加した。ほか、`web-authoring-set.el` の旧 `find` を `cl-find` へ移行、表変換ヘルパーをトップレベル関数化、ルビ除去正規表現を修正、Auto Complete の `defadvice` を `advice-add` へ移行、パス表示のformat文字列を安全化、シェル扱いするドットファイルを既知のシェル設定へ限定、PHPマニュアル言語を `ja` に更新した。
- なぜそうしたか: 暗黙依存や旧API、実動作不良、広すぎるファイルモード関連付けを解消し、異常終了・多重編集時のデータ保護を戻すため。また、一時ファイルの用途別整理と、安全に限定した自動清掃を両立するため。FTP資格情報処理は利用縮小中との判断により現状維持とした。
- 未完了の事項: 新しい保存・ガベージコレクション設定は次回Emacs起動後に実運用へ反映される。Auto Complete adviceの実際の補完操作と、各種ドットファイルの自動モード選択はGUI上では未確認。
- 次にやるとよいこと: Emacs再起動後に `.tmp/backups/`、`.tmp/auto-save/`、`.tmp/auto-save-list/`、`.tmp/junk/` が生成されることを確認し、Auto Complete確定時に余分な改行が入らないことを確認する。

- 何をしたか: `inits/search-center.el` の検索専用ミニバッファ設定で、現在のローカルキーマップを複製してから `<muhenkan>` などの専用キーを割り当てるようにした。
- なぜそうしたか: `local-set-key` が共有の `minibuffer-local-map` を直接変更し、HTMLタグ削除など通常のミニバッファでも `<muhenkan>` が検索中断コマンドとして動作していたため。
- 未完了の事項: なし。`M-s-r` のプロンプトで `<muhenkan>` を押してもプロンプトが維持されることを実機確認済み。
- 次にやるとよいこと: なし。

- 何をしたか: `inits/filer.init.el` に `my/find-file-dwim` を追加し、`C-x C-f` へ割り当てた。通常のパスは従来の `find-file-at-point` 相当へ委譲し、HTTP(S) URLはURL接頭辞ごとにセッション内で記憶したローカルルートへ変換して開く。初回の `URL root:` には、ファイル風URLなら親ディレクトリ、ディレクトリURLならURL自体を初期値として入れ、対応するローカルルートを尋ねる。複数の登録には最長接頭辞一致を使う。`C-u C-x C-f` で対応先を選び直せるようにし、ディレクトリURLでは `index.html` / `index.htm` を補完する。
- なぜそうしたか: Dockerで配信中の大量のHTMLをChromeで確認し、コピーしたURLから対応するローカルファイルを普段のファイルオープン操作で直接編集できるようにするため。クエリとフラグメントを除去し、パーセントデコード後の `..` とローカルルート外への解決を拒否して安全性も確保した。
- 未完了の事項: なし。GUI上で実URLを使う一連の操作が快適に動作することを確認済み。
- 次にやるとよいこと: なし。

- 何をしたか: `inits/editing.init.el` に `my/normalize-region-line-breaks` を追加し、`s-/` と `<s-kp-divide>` をこのコマンドへ割り当てた。空行も行頭 `- ` もない選択範囲は従来の `join-multi-lines-to-one` へ委譲し、構造を含む場合は連続する空行を1つへ縮約し、箇条書き項目の直前の改行を維持しつつ、それ以外の折り返しを連結するようにした。
- なぜそうしたか: `join-multi-lines-to-one` の「必ず1行にする」という名前と役割を変えず、文章の段落と箇条書き構造を保った整形を同じキー操作で利用できるようにするため。
- 未完了の事項: GUI上の実原稿による操作感の確認は未実施。
- 次にやるとよいこと: 実際の文章を選択して `s-/` を実行し、段落間の空行と箇条書きの区切りが意図どおりになることを確認する。

- 何をしたか: `inits/tramp.init.el` の誤ったバックアップ除外設定を、リモートファイルを明示的に拒否する `backup-enable-predicate` へ置き換えた。あわせて `remote-file-name-inhibit-locks` を有効にし、TRAMPの自動保存先をローカルの `.tmp/auto-save/` へ明示した。
- なぜそうしたか: `backup-directory-alist` でTRAMPパターンの保存先を `nil` にするとバックアップ禁止ではなく元ディレクトリへの保存となり、リモートサーバへ `foo.txt~` が作られ得たため。バックアップ、ロックファイル、自動保存のいずれもリモート側へ余計なファイルを生成しないようにするため。
- 未完了の事項: 既にリモートサーバ上へ生成されたバックアップファイルやロックファイルの調査・削除は行っていない。現在開いているTRAMPバッファには、Emacs再起動またはファイルの開き直しまでバックアップ除外判定が反映されない可能性がある。
- 次にやるとよいこと: Emacsを再起動し、TRAMPファイルを開いて保存した際にリモート側へ `~` バックアップや `.#` ロックファイルが生成されないことを確認する。既存ファイルを削除する場合は対象パスを確認してから個別に行う。

- 何をしたか: `inits/editing.init.el` から、ファイル名とロケールの文字コードをmacOS HFS向けの `utf-8-hfs` に固定する設定を削除した。Ubuntu標準の `utf-8-unix` によって、NFCの日本語ファイル名をそのまま解決できるようにした。
- なぜそうしたか: `utf-8-hfs` が濁点を結合文字へ分解し、Linux上にNFCで存在する `こごみネット` を、存在しないNFDの `こごみネット` として開いて空の新規バッファを作っていたため。本文を明示的にNFC正規化する `normalize-chars` の機能には影響させない。
- 未完了の事項: 動作中のEmacsには古い文字コード設定と、存在しないNFDパスを参照する空バッファが残っている。空バッファは保存しておらず、ディスク上の実ファイルも変更していない。
- 次にやるとよいこと: 空の対象バッファを保存せず閉じてEmacsを再起動し、NFC名の実ファイルを開き直す。

- 何をしたか: `my/normalize-region-line-breaks` に、選択範囲の全行が同じ深さの `>` 引用である場合だけ引用記号を維持して整形する分岐を追加した。折り返し行は連結し、引用内の空行と箇条書きは既存の正規化規則に沿って残す。引用行の先頭インデントは選択範囲内に限って除去する。
- なぜそうしたか: 引用文を1行へ整形した際に引用記号まで消える問題を解消し、通常文や引用と通常文が混在する選択範囲の従来動作を維持するため。
- 未完了の事項: GUIでの実キー操作による確認は未実施。コミット・push は未実施。
- 次にやるとよいこと: Emacsへ変更を読み込ませ、実際の引用文で `s-/` を確認する。問題がなければコミットする。
## 2026-09-25 ファイル名補完の重複を修正

- 何をしたか: `inits/filer.init.el` で `zlc-select-nth` を上書きし、候補の共通部分を `face` 属性から取得するようにした。
- なぜそうしたか: 現在の Emacs 30.2 のファイル名候補では `completion--unquoted` 属性が `face` より前に付くため、zlc 0.0.5 が `~/.co` に `.codex/` 全体を追記していた。
- 未完了の事項: いつから属性の並びが変わったかは未特定。利用者が実際の操作で修正を確認済み。
- 次にやるとよいこと: 他の補完候補で同様の問題が出た場合は、その候補の文字列属性を調べる。

## 2026-09-26 引用文の行頭を段階的に除去

- 何をしたか: `s-[` と `s-{` に、行頭のスペース・タブがあればすべて除去し、空白がなく `>` で始まる場合は `>` だけを1文字除去するコマンドを割り当てた。選択範囲の各行、選択なしなら現在行を処理する。選択末尾が次行の先頭ならその行は対象外とする。
- なぜそうしたか: メールの引用文から空白と引用記号を段階的に取り除くため。
- 検証: Emacsバッチで括弧整合性と5件の動作確認を実施し、すべて成功。利用者からも良好との回答を得た。
- 未完了の事項: なし。
- 次にやるとよいこと: 通常利用で操作感を確認する。
